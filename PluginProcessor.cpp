...
juce::AudioProcessorValueTreeState::ParameterLayout Api550bAudioProcessor::createParameterLayout()
{
    ...
    params.push_back(std::make_unique<juce::AudioParameterFloat>(Params::SAT_DRIVE, "Saturation Drive", 0.5f, 5.0f, 2.0f));
...
void Api550bAudioProcessor::prepareToPlay(double sampleRate, int samplesPerBlock)
{
    ...
    updateFilters(); // Initial update to set saturation
}
...
void Api550bAudioProcessor::updateFilters()
{
    ...
    float lowGain = getParamValue(Params::LOW_GAIN, gainDbValues, std::size(gainDbValues));
    ...
    lowFilter.coefficients = lowShelf
        ? juce::dsp::IIR::Coefficients<float>::makeLowShelf(sampleRate, lowFreq, lowQ, juce::Decibels::decibelsToGain(lowGain))
    ...
    float lmGain = getParamValue(Params::LM_GAIN, gainDbValues, std::size(gainDbValues));
    float lmQ = useProportionalQ ? juce::jlimit(0.7f, 2.2f, 1.0f + 0.2f * std::abs(lmGain)) : fixedQ;
    lowMidFilter.coefficients = juce::dsp::IIR::Coefficients<float>::makePeakFilter(sampleRate, lmFreq, lmQ, juce::Decibels::decibelsToGain(lmGain));

    // High-Mid Band
    float hmFreq = getParamValue(Params::HM_FREQ, highFreqValues, std::size(highFreqValues));
    float hmGain = getParamValue(Params::HM_GAIN, gainDbValues, std::size(gainDbValues));
    float hmQ = useProportionalQ ? juce::jlimit(0.7f, 2.2f, 1.0f + 0.2f * std::abs(hmGain)) : fixedQ;
    highMidFilter.coefficients = juce::dsp::IIR::Coefficients<float>::makePeakFilter(sampleRate, hmFreq, hmQ, juce::Decibels::decibelsToGain(hmGain));

    // High Band
    float highFreq = getParamValue(Params::HIGH_FREQ, highFreqValues, std::size(highFreqValues));
    float highGain = getParamValue(Params::HIGH_GAIN, gainDbValues, std::size(gainDbValues));
    ...
    highFilter.coefficients = highShelf
        ? juce::dsp::IIR::Coefficients<float>::makeHighShelf(sampleRate, highFreq * 1.3f, highQ, juce::Decibels::decibelsToGain(highGain))
